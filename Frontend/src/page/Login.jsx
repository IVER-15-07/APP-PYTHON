...
import { BotonLink, SocialButton, Spinner, InputField } from "../components/ui" 
...
const Login = () => {
  ...
  const getRolMessage = () => {
    if (rolParam === "usuario") return "Inicia sesión como usuario"
    if (rolParam === "estudiante") return "Inicia sesión como Estudiante"
    return "Loguéate para continuar"
  }
  ...
  const redirectByRole = (rolId) => {
    if ([1, 2, 3, 5].includes(rolId)) navigate("/profesor")
    else if (rolId === 4) navigate("/estudiante")
    else navigate("/login")
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 flex items-center justify-center p-5">
      <div className="relative bg-slate-800/90 backdrop-blur-lg rounded-2xl p-8 w-full max-w-md shadow-2xl border border-slate-700/50">
        <button
          onClick={() => navigate("/")}
          aria-label="Cerrar"
          title="Cerrar"
        ...
        <SocialButton onClick={() => handleFirebaseLogin("google")} loading={loading} ariaLabel="Continuar con Google" icon={
          ...
          {loading ? "Logueándote..." : "Continuar con Google"}
        </SocialButton>

        <SocialButton onClick={() => handleFirebaseLogin("microsoft")} loading={loading} ariaLabel="Continuar con Microsoft" icon={
          <svg className="w-5 h-5" viewBox="0 0 24 24" aria-hidden="true">
          ...
          {loading ? "Logueándote..." : "Continuar con Microsoft"}
        </SocialButton>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          >
            {loading ? (
              ...
              `Iniciar Sesión${rolParam ? ` como ${rolParam.charAt(0).toUpperCase() + rolParam.slice(1)}` : ""}`
            )}
          </button>
        </form>
        ...
        <div className="text-center mt-6">
          <p className="text-slate-400 text-sm">
            ¿No tienes cuenta?{" "}
            <Link to={rolParam ? `/register?rol=${rolParam}` : "/register"} className="text-emerald-400 hover:text-emerald-300 font-medium transition-colors duration-300 hover:underline">
              Regístrate{rolParam ? ` como ${rolParam}` : ""}
            </Link>
          </p>
          <div className="mt-3">
            <BotonLink to="/recover" variant="secondary">Olvidé mi contraseña</BotonLink>
          </div>
        </div>
      </div>
    </div>
