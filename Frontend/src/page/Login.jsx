...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
      ...
      if (response.success) {
        const user = response.data.usuario; // OBTENER USUARIO DEL BACKEND
        
        console.log('Usuario logueado:', user); // DEBUG
        
        // REDIRIGIR SEGÚN ROL A RUTAS PROTEGIDAS
        if (user.rol_usuarioId === 1) {
          navigate('/profesor'); // Profesor → VentanaProfesor
        } else if (user.rol_usuarioId === 2) {
          navigate('/estudiante'); // Estudiante → VentanaEstudiante
        } else {
          navigate('/login'); // Fallback si hay problemas
        }
      }
    } catch (error) {
    ...
    try {
      ...
      if(firebaseResult.success) {
        ...
        if (response.success) {
          ...
          //  REDIRIGIR SEGÚN ROL TAMBIÉN AQUÍ
          if (user.rol_usuarioId === 1) {
            navigate('/profesor'); // Profesor
          } else if (user.rol_usuarioId === 2) {
            navigate('/estudiante'); // Estudiante
          } else {
  ...
  return (
      ...
      <div className="bg-slate-800/90 backdrop-blur-lg rounded-2xl p-8 w-full max-w-md shadow-2xl border border-slate-700/50">
        ...
        >
          ...
          {loading ? 'Logueándote...' : // ✅ TÉRMINO CORRECTO
