...
function generateToken(user) {
  const payload = { id: user.id, email: user.email, rol: user.rol_usuario.nombre };
...
export const authService = {
  ...
  async login({ email, contrasena }) {
    ...
    if (user.provider !== "local") throw { status: 400, message: "Cuenta registrada con proveedor externo" };
