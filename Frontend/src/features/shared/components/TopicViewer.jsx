...
const TopicViewer = ({ role = 'student', backPath = null, fetchService = null }) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        // Usar el servicio personalizado si se proporciona, sino usar el predeterminado
        const serviceFn = fetchService || topicsService.getTopicStudents;
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-6 lg:p-10">
      <div className={`mx-auto ${useTwoColumnLayout ? 'max-w-7xl' : 'max-w-5xl'}`}>
        ...
        <div className="bg-slate-900/80 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-8 shadow-2xl">
          ...
          {/* Sección de comentarios - Solo visible para profesores */}
          {role === 'teacher' && <TopicComments topicId={id} />}
        </div>
      </div>
    </div>
  );
